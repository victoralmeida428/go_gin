...
func (repo *VariavelRepository) FindAll(id int) ([]model.ViewVariavel, error) {
	query := `
		SELECT
			variavel.id,
			tipo_variavel_id,
			...
			obrigatorio
		FROM
			indicadores.view_variavel variavel
			JOIN indicadores.tipo_variavel tipo ON tipo.id = tipo_variavel_id
			JOIN indicadores.variavel_formulario ON variavel_formulario.variavel_id = variavel.id
			JOIN indicadores.versao_formulario ON versao_formulario.id = variavel_formulario.versao_formulario_id
		WHERE (versao_formulario.id = $1 OR $1=0)
	`
	result, err := repo.connection.Query(query, id)
	if err != nil {
		return nil, err
	}
	defer result.Close()
